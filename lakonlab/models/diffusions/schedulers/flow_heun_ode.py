...
@dataclass
class FlowHeunODESchedulerOutput(BaseOutput):
    prev_sample: torch.FloatTensor


class FlowHeunODEScheduler(SchedulerMixin, ConfigMixin):
    ...
    def set_timesteps(
        ...
        terminal_sigma = torch.zeros(1, device=sigmas.device)
        timesteps = torch.cat([sigmas[:1], sigmas[1:].repeat_interleave(2)])
        self.timesteps = (timesteps * self.config.num_train_timesteps).to(device)
        self.sigmas = torch.cat([timesteps, terminal_sigma]).to(device)
    ...
    def _get_derivative(
        ...
        if prediction_type == 'u':
            return model_output
        return (sample - model_output) / sigma.clamp_min(eps)
    ...
    def step(
            ...
            prediction_type='u',
            eps=1e-6) -> Union[FlowHeunODESchedulerOutput, Tuple]:
        ...
        if self.state_in_first_order:
            ...
            derivative = self._get_derivative(
                model_output, sample, sigma, prediction_type=prediction_type, eps=eps)
            dt = sigma_to - sigma
            ...
            prev_sample = self.sample + derivative * self.dt
        ...
        return FlowHeunODESchedulerOutput(prev_sample=prev_sample)
